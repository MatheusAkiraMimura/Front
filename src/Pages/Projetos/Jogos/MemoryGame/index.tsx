...
const MemoryGame = () => {
  ...
  // UseStates
  const recordePontuacao = localStorage.getItem("recordeJogoMemoria");
  const [recorde, setRecorde] = useState(Number(recordePontuacao));
  ...
  const handleCardClick = (cardId: number) => {
    ...
    if (newFlippedCards.length === 2) {
      const [firstCard, secondCard] = newFlippedCards.map((id) =>
        newCardsState.find((card) => card.id === id)
      );
  ...
  useEffect(() => {
    ...
    if (
      score === animaisTerrestres.length / 2 &&
      elapsedTime &&
      !gameCompleted
    ) {
  ...
  useEffect(() => {
    if (gameCompleted === true) {
      ...
      let pontuacaoBase = (score * 8000) / (tempoPausado * 1.3); 
      ...
      if (pontuacaoTotal > Number(recordePontuacao)) {
        localStorage.setItem("recordeJogoMemoria", String(pontuacaoTotal));

        setRecorde(Number(pontuacaoTotal));
    ...
    >
      ...
      <Box mt="3rem">
        <TipoDeJogoMemoria
          ...
          bloquearClique={
            !isProcessingClick && flippedCards.length === 0 ? false : true
          }
        />
      </Box>
            ...
            {cards
              ...
              .map((card) => (
                <Box
                  ...
                  transform={
                    card.isFlipped ? "rotateY(180deg)" : "rotateY(0deg)"
                  }
                  onClick={() => handleCardClick(card.id)}
                  cursor="pointer"
                >
                  {card.image.startsWith("data:image/") && card.isFlipped ? (
                    <Image src={card.image} rounded="md" />
                  ) : (
                    ""
                  )}
                </Box>
